...
internal class VocabularyRepository internal constructor(
    context: Context,
    databaseName: String = "vocabulary.db",
) {
    ...
    suspend fun save(word: SavedWord): SavedWord =
    ...
    /** Import is atomic, cancellable, and publishes once, after commit. */
    suspend fun importWords(
        words: List<SavedWord>,
        replace: Boolean,
        preserveReviewHistory: Boolean = false,
    ): Int =
        withContext(Dispatchers.IO) {
            mutex.withLock {
                ...
                try {
                    words.forEach { word ->
                        ...
                        if (existing == null) {
                            ...
                            val replacement =
                                if (preserveReviewHistory) {
                                    word.copy(dueAt = existing.dueAt, intervalMs = existing.intervalMs)
                                } else {
                                    word
                                }
                            check(
                                db.update(
                                    "words",
                                    ContentValues().apply {
                                        put("payload", encodeWord(replacement).toString())
                                    },
                                    "id = ?",
                                    arrayOf(word.id),
                                ) == 1,
    ...
    companion object {
        ...
        fun get(context: Context): VocabularyRepository =
...
internal fun encodeWord(w: SavedWord): JSONObject =
    JSONObject().apply {
        put("id", w.id)
        put("word", w.word)
...
internal fun decodeWord(j: JSONObject): SavedWord =
    SavedWord(
        id = j.getString("id"),
        word = j.getString("word"),
        reading = if (j.isNull("reading")) null else j.getString("reading"),
        ...
        translatedSentence = if (j.isNull("translatedSentence")) null else j.getString("translatedSentence"),
