...
class CaptionTranslationCancellationTest {
    ...
    @Test
    fun twentyTwoHourVideoTranslatesOnlyTheCurrentWindowAndThenSleeps() = runBlocking {
        withStore { store ->
            ...
            val job = launch {
                translatePlaybackWindow(store, requests, { text -> inputs.add(text); "translated: $text" }) { snapshot, preparing ->
    ...
    @Test
    fun seekDropsTheOldQueueAndNeverPublishesItsLateResult() = runBlocking {
        withStore { store ->
    ...
    @Test
    fun pauseFinishesOnlyTheInFlightEntryAndResumeContinues() = runBlocking {
        withStore { store ->
            val requests = MutableStateFlow(CaptionPlaybackRequest(0, paused = false, enabled = true))
            val started = CompletableDeferred<Unit>()
            val release = CompletableDeferred<Unit>()
    ...
    @Test
    fun supersededSessionCannotPublishEvenIfProviderCompletesAfterCancellation() = runBlocking {
        withStore { store ->
            val requests = MutableStateFlow(CaptionPlaybackRequest(0, paused = false, enabled = true))
            val started = CompletableDeferred<Unit>()
            val release = CompletableDeferred<Unit>()
    ...
    @Test
    fun unknownPlaybackAndBackgroundDoNotStartWork() = runBlocking {
        withStore { store ->
            val requests = MutableStateFlow(CaptionPlaybackRequest())
            var count = 0
            val ready = CompletableDeferred<Unit>()
            val job = launch {
                translatePlaybackWindow(store, requests, { count++; it }) { _, preparing ->
                    if (!preparing) ready.complete(Unit)
                }
            }
            try {
